...
/**
 * Modified version from frameworks
 * provides extra callback after a scroll stops
 */
public class GestureDetector {
    ...
    public interface OnGestureListener {
        ...
        /**
         * Notified when an up event occurred at the end of a regular scroll
         * (neither tap nor fling)
         */
    ...
    /**
     * scale the slop for tap and double tap
     *
     * @param scale factor
     */
    public void setSlopScale(Context context, float scale) {
        final ViewConfiguration config = ViewConfiguration.get(context);
        int ts = (int) (config.getScaledTouchSlop() * scale + 0.5f);
        mTouchSlopSquare = ts * ts;
    }
     ...
     *              else false.
     */
    public boolean onTouchEvent(MotionEvent ev, int offX, int offY) {
        final int action = ev.getAction();
        final float y = ev.getY() + offX;
        final float x = ev.getX() + offY;
        ...
        case MotionEvent.ACTION_UP:
            ...
            if (mIsDoubleTapping) {
                ...
                if ((Math.abs(velocityY) > mMinimumFlingVelocity)
                    ...
                    handled = mListener.onScrollUp(ev);
                }
            }
